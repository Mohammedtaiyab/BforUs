<?php
require('header.php');
?>


<section class="breadcrumb-area banner-3">
    <div class="text-block">
      <div class="container">
        <div class="row">
          <div class="col-lg-12 v-center">
            <div class="bread-inner">
              <div class="bread-menu">
                <ul>
                  <li><a href="home.php">Home</a></li>
                  <li><a href="#">Contact</a></li>
                </ul>
              </div>
              <div class="bread-title">
                <h2>Contact</h2>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>
 <section class="team pad-tb">
    <div class="container">
      <div class="row justify-content-center">
        <div class="col-lg-5 col-md-5 bg-shape">
          <div class="full-image-card mt0 upset">
            <div class="image-div"><a href="#"><img src="images/Image-1.jpg" alt="team" class="img-fluid"/></a></div>
          </div>
        </div>
        <div class="col-lg-6 col-md-6">
          <div class="full-image-card mt0">
            <div class="info-text-block">
              <h4><a href="#">Mohammed Taiyab</a></h4>
              <p>Web Developer</p>
            </div>
            <div class="otherinfo">
              <p>I always had a Dream of Starting my own business and bring my Creations to Life. It's a Challenge to bring Dreams to life. I had lots of Difficulties and Barriers that stopped me from Building my Website, but now it's live and improving every day. I aim to bring your Dreams into Reality through my Dreams.</p>
              <div class="social-media-profile">
                <a href="mailto:mohammedbori44@gmail.com" target="blank"><i class="fa fa-envelope"></i></a>
                <a href="https://www.instagram.com/md.taiyab_/" target="blank"><i class="fa fa-instagram"></i></a>
                <a href="http://linkedin.com/in/mohammed-bori-519829172" target="blank"><i class="fa fa-linkedin"></i></a>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>

  <!--End Service-->
<?php require('contectsec.php'); ?>
	<!--Start Clients-->
<?php
require('footer.php');
?>
